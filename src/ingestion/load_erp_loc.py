...
SOURCE_FILE = Path('dataset/source_erp/LOC_A101.csv')
SOURCE_SYSTEM = "ERP"
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            cur.execute("""
                ...
                create temp table temp_erp_loc(
                    cid TEXT,
                    cntry TEXT
                ) ON COMMIT DROP
            """)
            ...
            cur.execute("""
                ...
                INSERT INTO bronze.erp_loc_a101 (cid,cntry,source_file, batch_id)
                select cid,cntry,%s,%s
                from temp_erp_loc
            ...
            bronze_erp_loc_rows = cur.fetchone()[0]

            if bronze_erp_loc_rows != temp_rows:
                raise RuntimeError(f"row count mismatch temp: {temp_rows} and bronze:{bronze_erp_loc_rows}")
